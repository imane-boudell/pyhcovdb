...
gene_file_fasta = open('sarscov2_genes_sequences.fasta', 'w')
protein_file_fasta = open('sarscov2_proteins_sequences.fasta', 'w')
workbook = xlsxwriter.Workbook('sarscov2.xlsx')
worksheet = workbook.add_worksheet()

print("Started processing of COV-2 sequences...")

list_count = 0
list_retmax = 0

try:
    handle = Entrez.esearch(db="nucleotide", term="SARS-CoV-2[Organism] AND genomic_dna[filter]")
...
try:
    if list_retmax < list_count:
        handle = Entrez.esearch(db="nucleotide", term="SARS-CoV-2[Organism] AND genomic_dna[filter]", retmax=list_count)
        rec_list = Entrez.read(handle)
except Exception as e:
    pass
...
print("Finished processing of COV-2 sequences...")
